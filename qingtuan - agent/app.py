...
# -------------------------- 配置区（从环境变量读取，不写死！） --------------------------
API_KEY = os.getenv("API_KEY")  # 从Streamlit Cloud环境变量读取，绝对不写在代码里
BASE_URL = "https://api.moonshot.cn/v1"  # KIMI地址（Kimi请改成https://api.moonshot.cn/v1）
MODEL = "moonshot-v1-8k"  # Kimi请改成moonshot-v1-8k
# -----------------------------------------------------------------------------------
...
# 页面标题
st.markdown('<div class="title">💚 共青团志愿服务助手</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">青春心向党 · 志愿建新功</div>', unsafe_allow_html=True)
st.markdown('<div class="intro-text">📚 为青年志愿服务提供文案、策划、宣传等智能支持，让青春在奉献中闪光！</div>', unsafe_allow_html=True)

# 快捷提问按钮（填充中间空白）
col1, col2, col3, col4 = st.columns(4)
with col1:
    if st.button("📝 写宣传文案", use_container_width=True):
        st.session_state.prompt = "帮我写一段社区志愿活动的宣传文案"
with col2:
    if st.button("📋 做活动方案", use_container_width=True):
        st.session_state.prompt = "帮我设计一个图书馆志愿活动方案"
with col3:
...
with col4:
    if st.button("📜 写活动通知", use_container_width=True):
        st.session_state.prompt = "帮我写一篇社区志愿活动通知"
